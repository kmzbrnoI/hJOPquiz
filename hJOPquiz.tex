...
\fi
{\small v2.3}}
\author{Jan Malina (jan.malina@kmz-brno.cz)}
\date{\today}
\maketitle
...
\item Předveďte řízení lokomotivy v~režimu \textit{posun}. Dbejte na řádné
...
\item Předveďte poznání trati, ukažte polohy jednotlivých návěstidel.
...
\item Předveďte řízení lokomotiv v~multitrakci.
\end{enumerate}
...
těchto režimů?
\solution{Jako vlak nebo jako posunový díl. Rozdílů je hodně – způsob
sjednávání jízdy, ne/funkčnost návěstního opakovače, maximální rychlost, chování
vlakového zabezpečovače. Je třeba znát specifika obou režimů.}
...
\item Popište jednotlivé funkce vlakového zabezpečovače.
\solution{V~režimu \textit{vlak}:
\begin{enumerate}[noitemsep]
\item kontrola nepřekročení povolené rychlosti vlaku,
\item zamezení nepovolenému rozjetí,
\item zamezení rozjetí nepovoleným směrem,
\item zamezení nepovolené změny směru.
\end{enumerate}
V~režimu \texttt{posun}: kontrola nepřekročení 40 km/h.
}

\item Popište princip kontroly povolené rychlosti vlakovým zabezpečovačem.
\solution{Pokud vlak nebo posunový díl překročí povolenou rychlost, musí
strojvedoucí snížit rychlost na aktuální povolenou nebo nižší rychlost. Při nesnížení
do limitu 3~sekund dojde k~nouzovému zastavení řízených vozidel. Tento postup zaručuje
neprojetí návěstidla s~návěstí zakazující jízdu.}
...
zaškrtnutí volby \textit{ruční řízení}, zvuky pouští výhradně strojvedoucí, jinak
výhradně počítač.}

\item Jak strojvedoucí pozná, kde s~vlakem zastavit a kam nesmí jet?
\solution{Strojvedoucí se řídí návěstmi přenášenými na návěstní opakovač, maximální
dovolenou rychlostí a dovoleným směrem vlaku. Pokud se s~vlakem blíží k~místu
zastavení, má zastavit v~momentě, kdy povolená rychlost vlaku klesne na 0 km/h.
Činnost vlakového zabezpečovače v~posunu je značně omezená, proto jízdu v~posunu
musí strojvedoucí vždy důsledně sjednat s~výpravčím (kdo, odkud, kam).}

\item Jaká je maximální rychlost posunu?
\solution{40 km/h. Při sunutí vozů nejvýše 30 km/h. Při sunutí je třeba jet
obzvláště opatrně a~pomalu, vagóny jsou oproti lokomotivě lehčí a proto mohou
...
opakovačem v~aplikaci. Dodržení maximální rychlosti je kontrolováno vlakovým
zabezpečovačem.}
...
\item V~jakých situacích kromě posunu se používá režim VZ \textit{posun}?
\solution{Typicky jde o~jízdu na přivolávací návěst a další nouzové úkony vždy
po souhlasu výpravčího. Při standardní jízdě vlaku je strojvedoucí povinen vlakový
zabezpečovač vždy provozovat v režimu \textit{vlak}.}

\end{enumerate}

\newpage

\section{S1 – výpravčí základní}

Opravňuje k~řízení menších stanic.

Pro tuto úroveň zaškolení je doporučeno mít zaškolení na úrovni S0. Výjimkou
je situace, kdy výpravčí používají k~řízení jízdy jiné prostředky než mobilní
aplikaci hJOPdriver.
...
serveru. Přihlaste se v~režimu čtenáře, předveďte přihlášení více panelů.
...
\begin{enumerate}[leftmargin=*]
\item Předveďte komunikaci se sousedními výpravčími (telefon, poslání zprávy).
\item Předveďte přepnutí řízení na místní a dálkový provoz.
\end{enumerate}
...
provádí.
\item Předveďte obsluhu rozpojovačů.
...
\item Předveďte posun ve stanici.
...
\item Předveďte násilné převzetí soupravy od strojvedoucího.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
\item Předveďte poznání trati, ukažte polohy jednotlivých návěstidel, typy
...
\symbol{symboly/prej2.png}{0.1}{Uzavřen povelem výpravčího.}
...
\begin{enumerate}[leftmargin=*]
\item Popište, za co jako výpravčí odpovídáte.
...
\newpage
\section{S1u – výpravčí řízení jízdy přes uLI}
...
prvního panelu na počítači výpravčího, při nechtěném ukončení lze znovu
...
\begin{enumerate}[leftmargin=*]
\item Předveďte zapnutí a~vypnutí kolejiště a~serveru.
...
kolej.
\item Předveďte multitrakci v~ručním ovladači.
